...
def jaccard_w(s, t, lower_case = True, alphanum_only = True):
    return jaccard(wordset(s, lower_case, alphanum_only), wordset(t, lower_case, alphanum_only))


def jaccard_g(s, t, gram_size, lower_case = True, alphanum_only = True):
    return jaccard(gramset(s, gram_size, lower_case, alphanum_only), gramset(t, gram_size, lower_case, alphanum_only))
...
def editsim(s, t):
    ...
    for i in range(1,n+1):
        for j in range(1,m+1):
           dist[i][j] = min(dist[i-1][j]+1, dist[i][j-1]+1, dist[i-1][j-1] if s[i-1] == t[j-1] else dist[i-1][j-1]+1)

    return 1- dist[n][m]*1.0/max(m, n)

# only keep alpha, and number
def alphnum(s):
    sep = re.compile(r"[\W]")
    items = sep.split(s)
...
# word set: split using white space
...
# n-gram: split based on the word-gram?
def gramset(s, gram_size, lower_case = True, alphanum_only = True):
    ...
    return [sList[i:i+gram_size] for i in range(ns)]
